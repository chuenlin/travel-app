...
### 頁面（5 個，以 Tab Bar 切換）
1. **行程列表首頁** — 多行程卡片、新增 / 刪除行程、匯入 JSON
2. **行程詳情** — 瀏覽 / 編輯模式切換、天氣卡、Day 切換、時間軸
3. **預訂清單** — 三狀態（booked/pending/none）、截止提醒
4. **費用記帳** — 多幣別、人頭分攤
5. **連結＋打包清單** — 重要連結收藏、可打勾清單、行程備份匯出
...
1. **模式切換列**（頂部 sticky）
   - 鎖頭 icon + 狀態文字 + 切換按鈕，一列顯示
2. **天氣卡** — 溫度 / 降雨機率 / 穿搭建議
3. **Day 橫向切換列** — 可捲動，當天 Day 高亮
4. **成員列表** — 頭像圓形 + 名稱
5. **景點時間軸** — 點擊展開備註、連結、Google Maps
   - Plan B badge（備案景點）
   - 預訂狀態 badge（⚠ 需訂 / ✓ 已訂）
6. **住宿 Bar**（當天有 type=hotel 的 event 才顯示）
   - 固定在景點列表最底部
   - 顯示：住宿名稱 + note 內容（含密碼）
7. **開車時間警告**（當天 drive_mins 總和 > 240 分鐘才顯示）
   - 橘色警告列，顯示預估總時數
8. **新增景點 FAB**（僅編輯模式顯示）
...
- **單一 HTML 檔案**：所有 CSS、JS 都在 `index.html` 內，不分拆
...
| Agent | 負責範圍 |
|-------|---------|
| `builder` | UI 開發、功能實作、localStorage 整合 |
| `reviewer` | Code review、效能、安全性、模式切換邏輯檢查 |
