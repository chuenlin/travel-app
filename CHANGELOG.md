## [v1.10.0] 2026-05-13
### 新增
- 天氣地點改為手動輸入：在編輯模式下，天氣欄位頂部新增「🌍 天氣地點」輸入框，輸入城市名稱後按 Enter 或失焦即儲存並觸發天氣查詢
- 天氣地點儲存於 `day.weather_city`，切換 Day 或重開行程皆保留

### 修改
- 未設定天氣地點時，不再顯示隨機假資料，改為顯示「無法取得天氣資訊」白底卡片
- 天氣查詢從 `day.label`（Day 標題）改為 `day.weather_city`（獨立欄位），避免地名與行程標題混用
- `_fetchWeatherForTrip` 移除不再使用的 `trip` 參數
- `toggleEditMode` 新增 `renderWeather(day)` 呼叫，確保切換模式時同步顯示 / 隱藏城市輸入框
- 機票新增 / 編輯儲存後自動依出發日期時間排序（無日期的機票排到最後）
- `sw.js` 快取版本升至 `v4`，確保更新後 `app.js` / `style.css` 不被舊快取卡住
